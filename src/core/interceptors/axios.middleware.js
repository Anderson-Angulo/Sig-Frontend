...
import { toastAction } from 'core/store/actions/toast.action';
    ...
    axios.interceptors.response.use(
        (res) => {
            ...
            if (res.data.status <= CoreConstants.HttpResponse.ERROR_TECHNICAL)
                dispatch(toastAction.error(res.status + " Se produjo un error técnico", "Código del error: " + res.data.transactionId));

            return res;
        },
        (err) => {
            dispatch(toastAction.error("Error", "Se produjo un error al intentar conectarse con el servidor"));
            debugger;
